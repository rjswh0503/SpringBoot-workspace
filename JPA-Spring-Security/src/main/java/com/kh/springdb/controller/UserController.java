...
public class UserController {
	...
	//1.비밀번호 확인이 일치하지 않을 때
	// bindingResult.rejectValue(a,b,c) 
	// a = 검증을 실패한 필드의 이름을 나타냄 밑에서는 password2가 검증에 실패했음을 의미
	// b = 검증에 실패할 경우 보여질 메세지를 담은 변수
	// c = b가 담고 있는 비밀번호가 일치하지 않는 에러 났을 경우 보여질 메세지 
	
		  if(!userCheckForm.getPassword().equals(userCheckForm.getPassword2())) {
		  ...
		  userService.create(userCheckForm.getUsername(), userCheckForm.getEmail(), userCheckForm.getPassword());
